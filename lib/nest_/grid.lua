...
_grid.numbertoggle = _grid.muxaffordance:new { edge = 1, fingers = nil, tdown = 0, count = { 1, 1 }, vlast = 0, v = { 0, 0 } }

_grid.numbertoggle.new = function(self, o) 
    o = _grid.muxaffordance.new(self, o)
    ...
    if axis.x and axis.y then o.v[1] = type(o.v[1]) == 'table' and o.v[1] or { x = 0, y = 0 } end
    if axis.x and axis.y then o.vlast = type(o.vlast) == 'table' and o.vlast or { x = 0, y = 0 } end
...
_grid.numbertoggle.input.muxhandler = _obj_:new {
    point = function(s, x, y, z) 
        local held = 0

        if z > 0 then 
            s.tdown = util.time()
        else held = util.time() - s.tdown end

        if s.edge == z then
            s.v[2] = toggle(s, s.v[2])
            return s.v, held
        end
    end,
    line = function(s, x, y, z) 
        ...
        if z > 0 then
            ...
            if s.edge == 1 then 
                ...
                if i ~= s.v[1] then 
                    ...
                    s.v[1] = i
                    s.v[2] = togglehigh(s, s.v[2])
                    return s.v, 0, i - s.vlast
                else
                    s.v[2] = toggle(s, s.v[2])
                    return s.v, 0, i - s.vlast
                end
...
_grid.numbertoggle.output.muxredraw = _obj_:new {
    point = function(s, g, v)
        local lvl = lvl(s, 1)
        if lvl > 0 then g:led(s.p_.x, s.p_.y, lvl) end
    end,
